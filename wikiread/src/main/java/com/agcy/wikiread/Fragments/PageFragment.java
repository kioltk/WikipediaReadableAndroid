...
public class PageFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        for(View view: parsedViews)
            contentView.addView(view);

        return rootView;
    }
    public void parseViews(){
        parsedViews = new Parser(context).parseWikitext(page.revisions.get(0).content);
    }
    public void fetchImagesUrls(){

        final ArrayList<PictureView> seekers = diveAndFindSeekers(parsedViews);
